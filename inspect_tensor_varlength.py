...
t_input_bound = t_input_filtered[t_input_filtered.map(len) <= 50]
t_input_bound = t_input_bound[t_input_bound.map(len) >= 10].reset_index(drop=True)

import random
resulting_set, language = prepareData(t_input_bound,"donald")

rando = random.choice(resulting_set)
print(rando)
[word for word in rando.split(' ')]
print(indexesFromSentence(language,rando))
print(tensorFromSentence(language, rando))
